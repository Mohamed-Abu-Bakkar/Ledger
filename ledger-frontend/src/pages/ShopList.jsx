...
const ShopList = () => {
  ...
  const handleTailorChange = async (shopId, orderIndex, value) => {
    try {
      await api.put(`/shops/${shopId}/orders/${orderIndex}`, {
        tailorName: value,
      });
  ...
  const calculateRemainingCredit = (shop) => {
    const debitTotal = shop.orders.reduce(
      (acc, o) => acc + Number(o.debitAmount || 0),
      0
    );
  ...
  return (
    <div className="flex items-center justify-center min-h-screen bg-white min-w-screen">
      <div className="flex items-center justify-center min-h-screen bg-green-100 w-full px-2 sm:px-0">
        ...
        <div className="w-full min-h-screen py-8 flex justify-center">
          <div className="max-w-6xl w-full mx-auto p-4 sm:p-6 bg-green-200 rounded-2xl shadow-2xl border border-green-400">
            ...
            {shops
              ...
              .map((shop, idx) => (
                <div
                  key={idx}
                  className="border border-green-300 rounded-2xl p-8 mb-10 shadow-xl bg-green-50"
                >
                  <div className="mb-4 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                    <div>
                      <p className="font-bold text-2xl text-green-800 mb-1">
                        {shop.shopName}
                      </p>
                  ...
                  <div className="space-y-6">
                    {shop.orders.map((order, i) => {
                      ...
                      return (
                        <div
                          key={i}
                          className={`border border-green-200 p-5 rounded-xl bg-green-100 shadow-md ${
                            isClosed ? "opacity-60 bg-gray-200" : ""
                          }`}
                        >
                          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                            ...
                            <div className="flex gap-4 flex-wrap mt-2 md:mt-0">
                              {[
                                ...
                                <label
                                  key={stage}
                                  className="text-sm capitalize text-green-900 flex items-center gap-1"
                                >
                                  <input
                                    type="checkbox"
                          ...
                          <div className="mt-4">
                            <input
                              ...
                              onBlur={(e) =>
                                handleTailorChange(shop._id, i, e.target.value)
                              }
                  ...
                  <div className="mt-8 bg-green-50 p-5 rounded-xl border border-green-300">
                    <p className="font-semibold text-green-800 mb-3 text-lg">
                      💰 Record Payment
                    </p>
                    <div className="flex flex-col sm:flex-row gap-2">
                      ...
                      <input
                        ...
                        className="border border-green-300 p-3 rounded-lg w-full max-w-2xl bg-green-50 text-green-900 placeholder-green-700 focus:ring-2 focus:ring-green-400 focus:border-green-400 transition outline-none shadow-sm"
                      />
                      <button
                        onClick={() => handlePaymentSubmit(shop._id)}
                        className="bg-green-600 text-white px-6 py-3 rounded-lg hover:bg-green-700 font-semibold shadow w-full sm:w-auto"
                      >
                        Save Payment
                      </button>
                    </div>
